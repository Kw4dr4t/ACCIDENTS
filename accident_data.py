...
df_map[['Latitude', 'Longitude', 'count']].groupby(
    ['Latitude', 'Longitude']).sum().sort_values(
        'count', ascending=False).head(10)

print(df1['type_of_injury'].unique())


def generate_base_map(default_location: object = [51.519120, 21.281183],
                      default_zoom_start: object = 12) -> object:
...
m = HeatMap(data=df_map[['Latitude', 'Longitude', 'count']].groupby(
    ['Latitude', 'Longitude']).sum().reset_index().values.tolist(), radius=7,
...
df2['count'] = 0
df5 = df2[['district_commune', 'count']].groupby(
    ['district_commune']).sum().sort_values('count', ascending=False)
